...
-- :l C:\Users\Daniel\Dropbox\Daniel\Development\human_sudoku_solver\main.hs
...
split_group_by_distinct group board = 
	partition (\(_,v)->length v == 1) (cells_in_group group board)

is_distinct :: Cell -> Bool
is_distinct (_,[v]) = True
is_distinct _ = False
...
positions_of_non_distincts board = [ p | (p,vs) <- board, length vs > 1]

onlyChoice :: Strategy
onlyChoice board = 
    [ (unalloc_pos d g, [unalloc_val d]) | g <- all_groups, let d = distincts_in_group board g, length d == 8 ]
    where 
        unalloc_pos :: [Cell] -> Group -> Position
        unalloc_pos cells g = head $ positions_in_group g \\ map cell_position cells
        unalloc_val :: [Cell] -> Int
        unalloc_val cells = head $ [1..9] \\ (concatMap cell_values cells)

row_values :: Position -> Board -> ValueSet
row_values (c1,r1,b1) board = concat $ map cell_values $ filter (\((c,r,b),_)->c==c1&&r/=r1) $ filter is_distinct board

col_values :: Position -> Board -> ValueSet
col_values (c1,r1,b1) board = concat $ map cell_values $ filter (\((c,r,b),_)->r==r1&&c/=c1) $ filter is_distinct board

block_values :: Position -> Board -> ValueSet
block_values (c1,r1,b1) board = concat $ map cell_values $ filter (\((c,r,b),_)->not(r==r1&&c==c1)&&b1==b) $ filter is_distinct board

singlePossibility :: Strategy
singlePossibility board = 
	[ (p, vs) | p <- positions_of_non_distincts board, let vs = [1..9] \\ (row_values p board ++ col_values p board ++ block_values p board), length vs == 1 ]
...
onlySquare board = concat $
	[ [(p1,vs1), (p2, vs2)] | 
			   ...
			   length vs1 == 1,
			   let vs2 = [1..9] \\ (vs1 ++ (concatMap cell_values d))
			   ]
	...
	where 
		char2cell (i,ch) = ((c,r,get_block c r), vs)
			where (c, r) = (i `mod` 9, i `quot` 9)
			      vs = if ch == '.' then [1..9] else [ord ch - ord '0']
...
test1 = onlyChoice sample1 == [((0,1,0),[4])]
test2 = singlePossibility sample2 == [((0,1,0),[9]),((2,2,0),[3]),((0,3,3),[7]),((3,3,4),[3]),((1,4,3),[3]),((0,5,3),[6]),((6,5,5),[3]),((5,6,7),[2]),((6,6,8),[8]),((2,7,6),[6])]
test3 = onlySquare sample3 == [((2,8,6),[1]),((2,0,0),[3])]